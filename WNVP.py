...
def load_weather():
    weather = {}
    for line in csv.DictReader(open(file_path+'weather.csv')):
...
def load_training():
    training = []
    for line in csv.DictReader(open(file_path+'train.csv')):
        for name, converter in {"Date" : date, 
...
def load_testing():
    training = []
    for line in csv.DictReader(open(file_path+'test.csv')):
        for name, converter in {"Date" : date, 
...
def assemble_X(base, weather):
    ...
    for b in base:
        ...
        #for days_ago in [1,2,3,4,5,6,7,8,12]:
...
def Actual_Data(net, mean, std):
    ...
    #
    out = csv.writer(open(file_path+"output/Roshan_submission_NN2.csv", "w"))
