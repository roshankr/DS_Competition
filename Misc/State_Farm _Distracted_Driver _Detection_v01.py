...
#######################################################################################################################
#Run from background
#nohup sh State_Farm\ _Distracted_Driver\ _Detection_v01.sh > ./temp/test.txt >&1 &
#######################################################################################################################
...
def get_driver_data():
    dr = dict()
    clss = dict()
    path = os.path.join(file_path, 'driver_imgs_list.csv')
...
def load_train(img_rows, img_cols, color_type=1):
    ...
    for j in range(1):
        #path = os.path.join(file_path,'imgs',train_folder, 'c' + str(j), '*.jpg')
        path = os.path.join(file_path, train_folder, 'c' + str(j), '*.jpg')
        files = glob.glob(path)
        print('Load folder c%s , Total files :- %d' %(format(j),len(files)))
        for fl in files:
...
def load_test(img_rows, img_cols, color_type=1):
    ...
    #path = os.path.join(file_path,'imgs',test_folder, '*.jpg')
    path = os.path.join(file_path,  test_folder, '*.jpg')
    files = glob.glob(path)
    X_test = []
    X_test_id = []
    total = 0
    ...
    print('Load test images... Total files :- %d' %(len(files)))
...
def read_and_normalize_test_data(img_rows, img_cols, color_type=1):
    cache_path = os.path.join(file_path+'cache', 'test_r_' + str(img_rows) + '_c_' + str(img_cols) + '_t_' + str(color_type) + '_rotated.dat')
...
def read_and_normalize_train_data(img_rows, img_cols, color_type=1):
    cache_path = os.path.join(file_path+'cache', 'train_r_' + str(img_rows) + '_c_' + str(img_cols) + '_t_' + str(color_type) + '_rotated.dat')
...
def save_useful_data(predictions_valid, valid_ids, model, info):
    ...
    suffix = info + '_' + str(now.strftime("%Y-%m-%d-%H-%M"))
    # Save predictions
    pred_file = os.path.join(file_path,'subm', 'data', 's_' + suffix + '_train_predictions.csv')
    result1.to_csv(pred_file, index=False)
    # Save model
    json_string = model.to_json()
    model_file = os.path.join(file_path,'subm', 'data', 's_' + suffix + '_model.json')
    open(model_file, 'w').write(json_string)
    # Save code
    cur_code = os.path.realpath(__file__)
    code_file = os.path.join(file_path,'subm', 'data', 's_' + suffix + '_code.py')
...
def run_cross_validation():
    ...
    print("Ending Model Cross Validation....... at Time: %s" %(tm.strftime("%H:%M:%S")))
...
def main(argv):
    ...
    if(platform.system() == "Windows"):
        ...
        file_path = 'C:\\Python\\Others\\data\\S_F_Distracted_Driver_Detection'

    else:
        file_path = '/mnt/hgfs/Python/Others/data/S_F_Distracted_Driver_Detection/'
